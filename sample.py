...
pytesseract.pytesseract.tesseract_cmd = r'C:\Users\Administrator\AppData\Local\Programs\Tesseract-OCR\tesseract.exe'  # installing tesseract ocr directory
...
while True:
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):  # press 'q' to capturing photo
        break
    cv2.imwrite('car1.jpg', image)  # it is save in this location
vid.release()
cv2.destroyAllWindows()

# now to read image file
image = cv2.imread('po.png')
...
for i in cnts:
    ...
    if (
            len(approx) == 4):  # 4 means it has 4 corner which well be most probably our number plate as it also has 4 corners
